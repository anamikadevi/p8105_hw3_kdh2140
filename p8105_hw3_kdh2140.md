...
variable. All variables are numeric, except for day and week\_day.
...
accel %>% 
  group_by(day, week) %>% 
  ggplot(aes(x = time_minute, y = activity_count, color = day_id)) +
  geom_line()
```

![](p8105_hw3_kdh2140_files/figure-gfm/activity_plot-1.png)<!-- -->
...
```
    ...
    ## Warning: Removed 6803 rows containing missing values (geom_point).

    ## Warning: Removed 6803 row(s) containing missing values (geom_path).
...
![](p8105_hw3_kdh2140_files/figure-gfm/unnamed-chunk-3-1.png)<!-- -->
...
![](p8105_hw3_kdh2140_files/figure-gfm/unnamed-chunk-4-1.png)<!-- -->

``` r
## need to change to geom_hex
```

ny\_noaa\_df %\>% drop\_na(tmax, tmin) %\>% ggplot(aes(x =
as.numeric(tmax), y = as.numeric(tmin))) + geom\_hex()
